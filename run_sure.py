...
class ACEDataset(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        print("proceeding file:", file_path)
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            self.candi_top_markerid2token = {}
            self.candi_worst_markerid2token = {}

            for i in range(len(self.label_list)):
                if(i < len(self.sym_labels)):
                    self.candi_top_markerid2token[i]    =  '[unused%d]' % (i + 5)
                    self.candi_worst_markerid2token[i]  =  '[unused%d]' % (i + 5)
                else:
                    symed_length = len(self.label_list) - len(self.sym_labels)
                    self.candi_top_markerid2token[i]                    = '[unused%d]' % (i + 5)
                    self.candi_worst_markerid2token[i]                  = '[unused%d]' % (i + 5 + symed_length)
                    self.candi_top_markerid2token[i + symed_length]     = '[unused%d]' % (i + 5 + symed_length)
                    self.candi_worst_markerid2token[i + symed_length]   = '[unused%d]' % (i + 5)
...
def train(args, model, tokenizer):
    ...
    ##########################################################
    top_markerid2token = train_dataset.candi_top_markerid2token
    worst_markerid2token = train_dataset.candi_worst_markerid2token
    ##########################################################
    ...
    print('warming up step : ', warming_step)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step >= warming_step:
                ...
                re_score    = outputs[3]
                ner_score   = outputs[4]
                n = args.candidate_top_n
                m = args.candidate_worst_m

            
                with torch.no_grad():
                    
                    dim0, dim1, dim2 = inputs['attention_mask'].size()
                    all_candidate_length = args.max_candidate_length * args.max_pair_length
                    res_attention_mask = torch.full((dim0, dim1+all_candidate_length, dim2+all_candidate_length), 0)
                    res_attention_mask = res_attention_mask.to(dtype=inputs['attention_mask'].dtype, device=args.device)
                    res_attention_mask[:, :dim1, :dim2] = inputs['attention_mask']
                    inputs['attention_mask'] = res_attention_mask
                    
                    dim0, dim1 = inputs['input_ids'].size()
                    res_input_ids = torch.full((dim0, dim1 + all_candidate_length), tokenizer.pad_token_id)
                    res_input_ids = res_input_ids.to(dtype=inputs['input_ids'].dtype, device=args.device)
                    res_input_ids[:, :dim1] = inputs['input_ids']
                    inputs['input_ids'] = res_input_ids
                    
                    dim0, dim1 = inputs['position_ids'].size()
                    res_position_ids = torch.full((dim0, dim1 + all_candidate_length), 0)
                    res_position_ids = res_position_ids.to(dtype=inputs['position_ids'].dtype, device=args.device)
                    res_position_ids[:, :dim1] = inputs['position_ids']
                    inputs['position_ids'] = res_position_ids
                    
                    
                    for bi in range(len(re_score)):
                        objs = re_score[bi]
                        len_text = inputs['L'][bi]
                        for x in range(len(objs)):
                            if inputs['labels'][bi][x] == -1:
                                break
                            tep = objs[x]
                            indexed = sorted(enumerate(tep), key=lambda x: x[1], reverse=True)
                            max_prob = indexed[:n]
                            min_prob = indexed[-m:]
                            candidate_marker = []
                            if n > 0:
                                max_idx = [max_prob[i][0] for i in range(n)]
                                for i in max_idx:
                                    candidate_marker.append(top_markerid2token[i])
                            if m > 0:
                                min_idx = [min_prob[i][0] for i in range(m)]
                                for i in min_idx:
                                    candidate_marker.append(worst_markerid2token[i])
                            input_marker = tokenizer.convert_tokens_to_ids(candidate_marker)
                            
                            tensor_input_candidate_marker = torch.tensor(
                                input_marker, device=args.device, dtype=inputs['input_ids'][bi].dtype)
                            st_idx, ed_idx = args.max_seq_length + args.max_pair_length * 2 + x * args.max_candidate_length, args.max_seq_length + args.max_pair_length * 2 + (x + 1) * args.max_candidate_length
                            
                            inputs['input_ids'][bi][st_idx : st_idx + n + m] = tensor_input_candidate_marker
                            
                            value_obj = torch.ones(n + m)
                            value_obj = value_obj.to(dtype=inputs['attention_mask'][bi].dtype, device=args.device)
                            inputs['attention_mask'][bi][args.max_seq_length + x][st_idx : st_idx + n + m] = value_obj
                            inputs['attention_mask'][bi][args.max_seq_length + args.max_pair_length + x][st_idx : st_idx + n + m] = value_obj
                            
                            value_obj1 = torch.ones(len_text)
                            value_obj2 = torch.zeros(args.max_seq_length-len_text)

                            value_obj3 = torch.zeros(args.max_pair_length*2)
                            value_obj3[x] = 1
                            value_obj3[x+args.max_pair_length] = 1

                            value_obj4 = torch.zeros(args.max_pair_length * args.max_candidate_length)
                            value_obj4[x*args.max_candidate_length : x*args.max_candidate_length+n+m ] = 1
                            
                            value_obj = torch.cat((value_obj1, value_obj2, value_obj3, value_obj4), dim=0)
                            
                            value_obj = value_obj.to(dtype=inputs['attention_mask'][bi].dtype, device=args.device)
                            inputs['attention_mask'][bi][st_idx : st_idx + n + m] = value_obj

                            value_obj = torch.ones(n + m)
                            value_obj = value_obj.to(dtype=inputs['attention_mask'][bi].dtype, device=args.device)
                            st = inputs['sub_positions'][bi][0]
                            ed = inputs['sub_positions'][bi][1]
                            
                            inputs['attention_mask'][bi][st][st_idx : st_idx + n + m] = value_obj
                            inputs['attention_mask'][bi][ed][st_idx : st_idx + n + m] = value_obj
                            
                            value_obj = torch.arange(args.max_seq_length+ x*(n+m), args.max_seq_length + (1 + x) * (n + m), 1)
                            value_obj = value_obj.to(dtype=inputs['position_ids'][bi].dtype, device=args.device)
                            inputs['position_ids'][bi][st_idx : st_idx + n + m] = value_obj
...
def main():
    ...
    parser.add_argument('--candidate_top_n', type=int, default=2, help="how many top n candidate do you want?")
    parser.add_argument('--candidate_worst_m', type=int, default=2, help="how many worst m candidate do you want?")
    parser.add_argument('--max_candidate_length', type=int, default=8, help="candidate max length")
